...
Einige langjährige GitHub Nutzer würden mich für diesen Gebrauch der Plattform stark kritisieren. Ich bin mir dessen bewusst, das einige von mir genuzte Funktionen von GitHub im speziellen, wie z.B. die Issues und Git im Allgemeinen nicht so gedacht sind, wie Ich sie hier genutzt habe. Unter [Probleme](#probleme) gehe ich teilweise näher darauf ein, da die spezielle Situatione diese Art der Verwendung nahelegt.
...
> Trotz der Ähnlichkeit ist das Spiel in keiner Weise mit Google verbunden, da der gesamte Code vbon mit als Eigenleistung Erschaffen wurde un in keiner Weise von dritten plagiiert wurde.
...
Es besteht die Möglichkeit den Schwierigkeitsgrad nch belieben anzupassen. dazu stehen flgende Parameter zur Verfügung:
...
Durch Klicken auf den zurücksetzen Button werden diese auf den Voreingestellten Wert gesetzt.
...
Außerhalb des Einstellungsfensters befindet sich am oberen Bildschirmrand das Symbol für den Steuerungsmodus. Dem Nutzer stehen insgesammt zwei Steuerungsmoi zu Auswahl. Standart ist die Maussteuerung, wo der Ballon dur das Bewegen der Maus oder das Wischen mit dem Fnger gesteuert wird.  
...
Wird eine Blase berührt, werden dem Score 5 Punkte gutgeschrieben und sie verschwindet. Beim berühren eines Hindernisses platzt der Ballon und das Spiel ist vorbei  und kann nun durch klick auf "Spielen" neu gestartet werden.  
...
Während des Spiels wird auf Basis der eingestellten Leveldauer und dem aktuellen Score das Level berechnet. Je höher das Level, desto schneller wird das Spiel im Verhällniss num ersten Level.
...
Entweder der Spielerhat verloren oder gewonnen.  
Verloren hat, wer ohne Superballon ein Hinderniss berührt.  
Gewinnen ist hingegen auf zwei Arten möglich. Ereicht der Spieler Level 50 hat er gewonnen. Bei Standerteinstellungen wird das Spiel nähmlich kurtz darauf zu schnell und unspielbar. Darum gibt es eine zweite Option um zu gewinnen. wurde die Geschwindigkeit erhöht, so ist die GEschwindigkeit aus LEvel 50 bereits früher erreicht. In diesen Fällen hat der Spieler gewonne, sobald er die GEschwindigkeit aus Level 50 erreicht.
...
Leider sihabennd diese Figuren untereinander nur shr eingeschränkte Kommunikationsmöglichkeiten, denn obwohl es die Möglichkeit "Eigene Blöcle" - also Funktionen, zu erstellen, können diese nicht außerhalb der Figur genztzt werden.
...
keine elegante und zugleich simple Lösung gibt, da beide Figuren initialisiert werden, sobald das Programm startet, beide Figuren initialisiert werden und es reiner Zufall ist, ob zuerst A oder B die kritische Stelle durchlaufen.  
