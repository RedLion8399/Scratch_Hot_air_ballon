...
## :balloon: About the project

This programm was created as a project for my computer science class at school in the 9th grade. This program was created as a project for my computer science class in 9th grade. For most of us, it was the very first program we had ever written. Therefore we used scratch as language for this project. However, it was not my first program, since I prefer Python over Scratch. With this prior knowledge, I decided to plan a larger project, which would had been difficult to manage without version control. This lead to Scratch-specific problems which are described in the following [problems](#bug-probleme) section.

Additionally hosting the repo on GitHub offered the advantage of being able to present my teacher the complete history in order to disprove the reproach of using stolen code. This repo contains all files relevant for the final assessment including the [programm flowchart](https://github.com/RedLion8399/Scratch_Hot_air_ballon/blob/master/PAP_1.04.pdf). This and further Scratch projects can be found in the following [Studio](https://scratch.mit.edu/studios/34356533).

## :books: Inhaltsverzeichniss

- [Scratch Heißluftballon](#scratch-heißluftballon)
  - [:balloon: About the project](#balloon-about-the-project)
  - [:books: Inhaltsverzeichniss](#books-inhaltsverzeichniss)
  - [:bulb: Inspiration](#bulb-inspiration)
  - [:star: Funktionen](#star-funktionen)
    - [:gear: Einstellungen](#gear-einstellungen)
    - [:video\_game: Spielablauf](#video_game-spielablauf)
    - [:checkered\_flag: Spielende](#checkered_flag-spielende)
  - [:hammer: Instalation / Anwendung](#hammer-instalation--anwendung)
  - [:bug: Probleme](#bug-probleme)
    - [:floppy\_disk: Speicherung und Versionierung](#floppy_disk-speicherung-und-versionierung)
    - [:warning: Eigenheiten Im Programmablauf](#warning-eigenheiten-im-programmablauf)
  - [:handshake: Danksagung](#handshake-danksagung)
  - [:scroll: Lizenz](#scroll-lizenz)
...
Bis zur Version [0.10.1](https://github.com/RedLion8399/Scratch_Hot_air_ballon/blob/master/Hot_air_ballon_0.10.1.sb3) lag das Augenmerk Hauptsächlich darauf, das Originalspiel möglichst genau nachzubauen. Aufgrund einiger Einschränkungen durch [Scratch](#bug-probleme) kommt diese Version dem "Original" am nächsten.
...
Der verbliebene Menüpunkt schaltet die Entwicklereinstellungen frei. Diese sind zum Testen des Spiels gedacht. Deren Funktion wird unter [Spielablauf](#video_game-spielablauf) näher erläutert.
...
### :floppy_disk: Speicherung und Versionierung

Wie eingangs angedeutet führt die Verwendung von Scratch als Programmiersprache zu einigen Problemen. Um Scratch Dateien zu editieren gibt es zwei Methooden: Die simpeste Variante besteht darin, die Projekte im Browser zu editieren und bei Scratch zu speichern. Diese Variante bietet sich vorallem an, wenn das Projekt öffentlich ist, da solche ohnehin online gespeichert werden müssen. In diesem Fall kann es aber z.B. aufgrund einer schlechten WLAN Verbinding zu Absturtz der Seite kommen, was einen Verlust der Projetdaten zur Folge haben kann.
...
Diese Form der Dateinspeicherung in Form binären Dateien erschwert eine Versionierung mittels Git deutlich und führt zu jener Form, die in diesem Repository zu sehen ist.

### :warning: Eigenheiten Im Programmablauf
